...
async function main() {
    ...
    JSON.stringify(
      {
        ...
        locales: locales.map((l) => ({ code: l.code, default: l.default })),
        selected: { defaultLocale, es: localeEs, en: localeEn },
      },
      null,
      2,
    ),
  ...
  const { contentfulUpsertContent, contentfulUpsertSiteSettings } = await import(
    "../server/contentful-store"
  );

  const SEO: Record<Locale, any> = {
    es: {
  ...
  const HEADER: Record<Locale, any> = {
    es: {
  ...
  const SPACES: Record<Locale, any> = {
    es: {
  ...
  const THERAPIES: Record<Locale, any> = {
    es: {
  ...
  const SERVICES: Record<Locale, any> = {
    es: {
  ...
  const PROCESS: Record<Locale, any> = {
    es: {
  ...
  const CONTACT: Record<Locale, any> = {
    es: {
  ...
  const FOOTER: Record<Locale, any> = {
    es: {
      text: "© 2025 DBT Salud",
  ...
  const CONTENT: Record<string, Record<Locale, any>> = {
    "luminous.seo": SEO,
  ...
  const localesToSeed: Locale[] = ["es", "en"];
